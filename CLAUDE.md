...
- **Layout:** top-text layout. `Fit` keeps margins; `Fill` enlarges the screenshot below the text (aspect ratio preserved, headline never covered). `Trim top` crops the screenshot's top (source rect) to remove the simulator status bar; rounded corners reapply to the new top edge. **Device frame** (default off) wraps the shot in a generic rounded bezel drawn in canvas (no Apple artwork), shrinking the shot so frame+screen stay inside the region; it renders in exports too.
