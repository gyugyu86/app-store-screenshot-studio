...
- **Project save/load (no deps):** `saveProject()` serializes the deck to one `shotsmith_project.json` — per-locale text + each screenshot re-encoded as a PNG data URL via canvas (so drag-drop `blob:` URLs become portable) + `style`/`locSel`/device/format/locale. `loadProject()` rehydrates and `syncControls()` pushes the restored style into every DOM control (segments, colour inputs, sliders + value labels). No compression.  
...
- **Layout:** top-text layout. `Fit` keeps margins; `Fill` enlarges the screenshot below the text (aspect ratio preserved, headline never covered). `Trim top` crops the screenshot's top (source rect) to remove the simulator status bar; rounded corners reapply to the new top edge. **Device frame** (default off) wraps the shot in a generic graphite rounded bezel drawn in canvas (no Apple artwork), width adjustable via `frameW` (default 1.8% of screen width); it shrinks the shot so frame+screen stay inside the region and renders in exports too.
- **Compose options (all in `state.style`, so save/load round-trips them):** background can be gradient / solid / **uploaded image** (cover; stored as a data URL, drawn via a cached element); a **logo/icon** overlay (uploaded PNG) sits centred above the title, size-adjustable; the device frame has a selectable **colour** (graphite/black/silver/white) with a luminance-picked inner edge; title and subtitle take **independent Latin fonts** (`fontFamily` / `subFont`; CJK still maps to bundled Noto) and **independent weights** (`titleW` / `subW`: Regular/Medium/Bold/Black). Weight is set on `ctx.font`, so it applies to CJK too — the bundled Noto JP/KR are variable, so no extra fonts (the load gate preloads 900). *(Adding more bundled CJK typefaces would be heavy — ~3–4MB each — so we vary CJK by weight, not by face.)*
...
- Google Play **Android phone \= 1080×1920** (9:16) — a 3rd device. Play accepts sides 320–3840px, ratio ≤2:1; we emit 1080×1920 via the same no-alpha encoder. Per-device loops use the shared `DEVS = Object.keys(DEVICES)`.
- More export sizes (all reuse the pipeline): **iPhone 6.7″ 1290×2796**, **Android tablet 1600×2560**, **Mac 2880×1800** (landscape), plus Google Play **feature graphic 1024×500** — a centred title/subtitle/logo *banner* (no screenshot; exports on title text; its include checkbox defaults off). The device UI (toggle, drop zones, status dots, export boxes) is generated/looped from `DEVS`.
- **UI layout:** three-panel workspace — left = content (locale tabs, text, screenshot drops, slides), centre = the large preview, right = style (background, text, fonts, logo, layout, project). One-row header: mobile devices (iPhone/iPad/Android) inline, the rest (6.7″/tablet/Mac/feature) under a "More formats" dropdown; the per-device/-language export checkboxes live in an "Export scope" popover. Screenshot drops group the same way (mobile inline, more collapsed). *(ES/FR/DE/PT output languages were trialled and removed — kept EN/JA/KR for simplicity; revisit on demand.)*  
