...
- **2026-06-22 — Device frame = generic canvas bezel, no assets/branding.** Default OFF. When on, `renderTo` draws a dark rounded rectangle (`#0B0B0D` + a faint inner white edge so it reads on dark backgrounds) around the screenshot and shrinks the shot by a fit factor so `frame+screen` stays inside the layout region (bezel ≈ 3.5% of screen width). No notch/camera/logo — deliberately generic to avoid Apple's bezel-artwork license in an MIT repo. Shares the same `renderTo` as export, so framed shots export identically (still alpha-free). Frame-off path is unchanged (bezel width 0 ⇒ outer rect == screen rect).
