...
1. **Optional device-frame toggle.** Default OFF. Draw a **generic** rounded bezel in canvas (no Apple trademarks/artwork — Apple's official bezels are under a marketing license and shouldn't be redistributed in an MIT repo) or accept a user-supplied frame PNG. *Difficulty: medium.*
